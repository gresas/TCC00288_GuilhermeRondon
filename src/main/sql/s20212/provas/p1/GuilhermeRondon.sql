...
-- create function resultado(p_pesquisa int, p_bairros varchar[], p_cidades varchar[]) 
--     returns int, float[]
--     language plpgsql as $$
--     declare 
--     begin
...
CREATE OR REPLACE FUNCTION resultado(p_research int, p_neighbr varchar[], p_cities varchar[])
RETURNS TABLE(pergunta_ int, histograma float[])AS $$
    DECLARE
        currentChoice RECORD;
        neighbr CURSOR FOR SELECT * FROM bairro WHERE numero = ANY(p_neighbr);
        cities CURSOR FOR SELECT * FROM cidade WHERE numero = ANY(p_cities);
        questions CURSOR FOR SELECT * FROM pergunta WHERE pesquisa = p_research;
        aux int[];
        answerCount int default 0;
    BEGIN
        FOR currQuestion IN questions LOOP
            SELECT COUNT(*) FROM resposta WHERE pesquisa = p_research AND pergunta = currQuestion.numero INTO answerCount;
            SELECT array_fill(0, ARRAY[answerCount]) INTO aux;
